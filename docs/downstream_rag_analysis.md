...
A downstream prompt ablation / generation-format sensitivity check for both Qwen2.5-1.5B-Instruct and Qwen2.5-7B-Instruct is reported in [downstream_rag_prompt_ablation_2x2.md](downstream_rag_prompt_ablation_2x2.md). It keeps the rankings, top-3 contexts, question set, context order, and decoding settings fixed, and only changes the prompt style within each generator. The earlier 1.5B-only slice is retained in [downstream_rag_prompt_ablation_1_5b.md](downstream_rag_prompt_ablation_1_5b.md).
...
- The strict-prompt ablations are downstream extension experiments, not reranking main results.
