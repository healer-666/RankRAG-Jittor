...
## Qwen2.5-1.5B LoRA Reranker Formal Result

The formal LoRA experiment uses `Qwen/Qwen2.5-1.5B-Instruct` as a log-probability reranker:

```text
score = log P("Relevant") - log P("Irrelevant")
```

It is evaluated on the same MS MARCO medium test candidate set as the BM25, Jittor MLP/TextCNN, JittorLLM zero-shot, and Cross-Encoder reference runs.

| Method | R@1 | R@3 | R@5 | R@10 | NDCG@5 | MRR | Pairwise Acc. |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| BM25 | 0.2300 | 0.5540 | 0.7840 | 1.0000 | 0.5074 | 0.4476 | 0.6253 |
| Jittor MLP | 0.2280 | 0.5060 | 0.7120 | 1.0000 | 0.4698 | 0.4318 | 0.5901 |
| Jittor TextCNN | 0.1800 | 0.4500 | 0.6780 | 1.0000 | 0.4270 | 0.3912 | 0.5484 |
| Qwen2.5-1.5B Jittor zero-shot | 0.2360 | 0.5520 | 0.8120 | 1.0000 | 0.5210 | 0.4525 | 0.6342 |
...
The LoRA v3 run uses 10,000 train pairs, 1,000 valid pairs, 800 train steps, max length 256, LoRA rank 8, and learning rate 1e-4. Its training loss decreases from 3.5866 to 0.0878, while the final claim is based on test ranking metrics rather than loss alone.

The result should be interpreted conservatively:

- LoRA v3 is clearly stronger than the same-size Qwen2.5-1.5B Jittor zero-shot reranker, showing the value of task-specific reranker training.
- LoRA v3 is stronger than BM25, Jittor MLP, and Jittor TextCNN on this candidate set.
- LoRA v3 remains below the external Cross-Encoder reference, so the correct conclusion is that LoRA narrows the gap rather than matching or surpassing the Cross-Encoder.
- This is still a RankRAG-style context ranking reproduction, not a full reproduction of RankRAG instruction tuning and answer generation.

Detailed artifacts are recorded in `docs/lora_qwen2_1_5b_results.md` and generated by `python src/aggregate_lora_results.py`.
